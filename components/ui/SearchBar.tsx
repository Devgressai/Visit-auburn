      ...
      <form onSubmit={handleSubmit} className={`max-w-3xl mx-auto ${className}`}>
        <div className="relative">
          <input
            ...
            className="w-full px-6 py-5 pr-28 text-lg rounded-2xl shadow-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/50 transition-all"
          />
      ...
      <input
        ...
        className={`pl-10 pr-4 py-2 rounded-full transition-all focus:outline-none focus:ring-2 ${
          isHomepage
            ...
            : 'bg-gray-100 text-gray-900 placeholder-gray-500 focus:bg-white focus:ring-blue-500 border border-gray-200'
        } ${isFocused ? 'w-64' : 'w-48'}`}
      />
      <Search className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${isHomepage ? 'text-white/70' : 'text-gray-400'}`} />
      
      {query && (
        <button
          type="button"
          onClick={handleClear}
          className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 ${isHomepage ? 'text-white/70' : 'text-gray-400'}`}
        >
          <X />
        </button>
      )}
