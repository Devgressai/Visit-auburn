...
import { thingsToDoItems, getItemBySlug, getCategoryBySlug } from '@/lib/thingsToDo.data'
...
export default async function AttractionDetailPage({ params }: { params: Promise<{ subHub: string; slug: string }> }) {
  const { subHub, slug } = await params
  const item = getItemBySlug(slug)
  const category = getCategoryBySlug(subHub)

  if (!item || !category || item.categorySlug !== subHub) {
    notFound()
  }
  ...
  const breadcrumbItems = [
    ...
    { label: item.title, href: `/things-to-do/${subHub}/${slug}` },
  ]
  ...
  const breadcrumbSchema = breadcrumbJsonLd([
    ...
    { name: item.title, url: `${SITE_URL}/things-to-do/${subHub}/${slug}` },
  ])
  ...
  const attractionSchema = touristAttractionJsonLd({
    ...
    url: `${SITE_URL}/things-to-do/${subHub}/${slug}`,
  })

  // Get related items from same category
  const relatedItems = thingsToDoItems
    .filter(i => i.categorySlug === subHub && i.id !== item.id)
    .slice(0, 3)
